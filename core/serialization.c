...
DECLARE_SERIALIZE_FUNCS_W_CAL_TZ(al,TripAlight*);
...
bool gDeserialize(Graph *g, char* gbin_name, char * mmf_name) {
	...
	long num_vertices = 0;
	long num_edges = 0;
	long num_calendars = 0;
	long num_timezones = 0;
	...
	for (i = 0; i < num_vertices; i++) {
		...
		while (out) { //while we have any more outgoing edges we want to serialize on this vertex
			...
			switch (p->type) {
				...
				case PL_ALIGHT: __SERIALIZE(al, TripAlight*, p); break;
	...
	switch (payload->type) {
		...
		case PL_TRIPBOARD:
			...
			*sc = ((TripBoard*)payload)->calendar;
			break;
		case PL_ALIGHT:
			*tz = ((TripAlight*)payload)->timezone;
			*sc = ((TripAlight*)payload)->calendar;
			break;
...
void test_serialize(char* filename) {
	...
	FWRITE_TYPE(l, long);
	streetSerialize(streetNew("A street", 1, 1), f, NULL);
	fclose(f);
	f = fopen(filename, "rb");
	FREAD_STRING(buff);
	l = 0;
...
void
alSerialize(TripAlight* a, FILE* f, FILE * mmf) {
	BUFF_SIZE;
...
alDeserialize(ServiceCalendar* calendar, Timezone* timezone, FILE* f, void* mm_data) {
	if (mm_data) {
		...
		TripAlight* al = alNew(sid, calendar, timezone, agency);
